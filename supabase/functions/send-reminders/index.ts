...
Deno.serve(async (req) => {
  ...
  if (isTest) {
    ...
    if (userErr || !userData.user) {
      return new Response(
        JSON.stringify({ error: userErr?.message ?? "Invalid token" }),
        {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
  ...
  for (const r of reminders ?? []) {
    let local;
    try {
      local = localPartsForTz(r.timezone || "UTC", now);
    } catch {
      local = localPartsForTz("UTC", now);
    }

    if (local.hhmm !== r.time_of_day) {
      skipped++;
      continue;
    }
    if (!r.days_of_week?.includes(local.dow)) {
      skipped++;
      continue;
    }
    if (r.last_sent_date === local.date) {
      skipped++;
      continue;
    }
    ...
    if (!dueTasks.length) {
      ...
      await supabase
        .from("notification_reminders")
        .update({ last_sent_date: local.date })
        .eq("id", r.id);
      skipped++;
      continue;
    }
